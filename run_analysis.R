# download UCI HAR Dataset (zip file) from 
...
# add subject_id and activity labels to extracted variables:
newcolumns <- c("subject_id", "activity", meanStd_vars) 
# add subject_id and activity labels to column labels:
columnnames <- c("subject_id", "activity", meanStd_labels) 
...
meanStd.variable.identifiers <- data_frame(colIndex=meanStd_index, Vnumber=meanStd_vars, 
                                featureName=meanStd_labels, newVariableName=new_colnames[3:81]) 
# creates dataframe showing correlation between original column indices (colIndex), 
# labels in original dataset (Vnumber), feature names (featureName), and new variable names (newVariableName)
write.csv(meanStd.variable.identifiers, "variable_identifiers.csv", row.names = FALSE) # save as .csv file
